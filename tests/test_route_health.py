...
from sparrow.proxy import _readiness_snapshot, _status_payload
from sparrow.quota import QuotaStore
from sparrow.route_health import FailureUpdate, RouteHealthStore
...
def test_half_open_lease_is_single_across_real_processes(tmp_path):
    ...
    import sparrow.route_health as route_health_module
...
def test_missing_fchmod_and_non_fcntl_instances_remain_safe(
    ...
    import sparrow.route_health as route_health
...
def test_default_pool_wires_configurable_persistent_health_path(tmp_path):
    ...
    pool = Pool.from_default_config(
        env={
            "SPARROW_HEALTH_FILE": str(path),
            "SPARROW_CONFIG": str(tmp_path / "missing.toml"),
        }
    )
...
def test_route_health_child_hook_replaces_inherited_path_locks():
    from sparrow import route_health as health_module
