...
from sparrow.errors import AllProvidersExhausted, NoProvidersConfigured
...
def test_pool_owned_quota_uses_bounded_success_batch_defaults(tmp_path, monkeypatch):
    ...
    configured_pool = Pool(
        [],
        env={
            "SPARROW_QUOTA_FLUSH_EVERY": "7",
            "SPARROW_QUOTA_FLUSH_INTERVAL": "0.25",
        },
    )
...
def test_sparrowerror_is_public_base_without_legacy_alias():
    import sparrow.errors as errors
    from sparrow import SparrowError
    from sparrow.errors import AllProvidersExhausted

    assert issubclass(AllProvidersExhausted, SparrowError)
    assert not hasattr(errors, "BuffetError")
...
def test_retry_after_sleep_is_deferred_until_distinct_provider_attempt(
    ...
    monkeypatch.setattr(client_module, "_post_once", post_once)
    monkeypatch.setattr(client_module, "_RETRY_BACKOFF_S", 0.0)
    monkeypatch.setattr("sparrow.router.time.sleep", lambda delay: events.append("sleep"))
    pool = Pool(_diversity_providers(), quota=quota, env={})
