...
def test_import_external_provider_to_user_catalog(tmp_path, monkeypatch):
    from sparrow.catalog import default_external_catalog_path
...
def test_import_external_provider_missing_cache_points_to_catalog_sync(tmp_path, monkeypatch):
    monkeypatch.setenv("SPARROW_EXTERNAL_CATALOG_PATH", str(tmp_path / "missing.json"))

    with pytest.raises(ValueError, match="sparrow catalog sync"):
