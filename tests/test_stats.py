...
def test_reads_old_minimal_file_and_stamps_version(tmp_path):
    ...
    p = tmp_path / "s.json"
    p.write_text('{"requests": 5}', encoding="utf-8")                               
    s = StatsStore(p)
    snap = s.snapshot()
...
def test_preserves_unknown_future_fields_on_roundtrip(tmp_path):
    ...
    saved = json.loads(p.read_text(encoding="utf-8"))
    assert saved["future_field"] == "keep me"                                          
    assert saved["version"] == 999                                             
...
def test_default_path_is_install_independent(monkeypatch):
    from sparrow.stats import default_stats_path

    monkeypatch.delenv("SPARROW_STATS_PATH", raising=False)
    p = default_stats_path()
    assert p.name == "stats.json"
                                                                                  
    assert str(Path(".config") / "sparrow") in str(p)
    assert "site-packages" not in str(p)
    monkeypatch.setenv("SPARROW_STATS_PATH", "/tmp/x/custom.json")
    assert default_stats_path() == Path("/tmp/x/custom.json")
...
def test_batched_stats_flush_merges_real_processes(tmp_path):
    ...
    import sparrow.stats as stats_module
