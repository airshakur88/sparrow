...
from sparrow.models import Model, Provider, Reply


def test_cli_start_parser_accepts_start_and_rejects_proxy():
    parser = build_parser()
...
def test_cli_models_json_is_machine_readable(monkeypatch, capsys, tmp_path):
    ...
    from sparrow.conformance import FEATURE_TOOLS, STATUS_PASS, ConformanceStore
    ...
    monkeypatch.setattr("sparrow.cli.load_catalog", lambda: catalog)
    monkeypatch.setattr(
        "sparrow.cli.configured_providers",
    ...
    monkeypatch.setattr("sparrow.cli.ConformanceStore", lambda: store)
...
def test_cli_models_and_conformance_include_plugin_providers(
    ...
    monkeypatch.setattr("sparrow.cli.load_catalog", lambda: [builtin])
    ...
    monkeypatch.setattr("sparrow.cli.ConformanceStore", lambda: store)
    monkeypatch.setattr("sparrow.cli.run_target_canaries", fake_run)
...
def test_cli_conformance_run_is_bounded_machine_readable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr("sparrow.cli.load_catalog", lambda: [provider])
    monkeypatch.setattr("sparrow.cli.configured_providers", lambda catalog, env=None: catalog)
    monkeypatch.setattr("sparrow.cli.ConformanceStore", lambda: store)
    monkeypatch.setattr("sparrow.cli.run_target_canaries", fake_run)
...
def test_cli_conformance_normal_mode_never_selects_disabled_model(monkeypatch, capsys):
    ...
    monkeypatch.setattr("sparrow.cli.configured_providers", lambda catalog, env=None: catalog)
    monkeypatch.setattr(
        "sparrow.cli.run_target_canaries",
...
def test_cli_conformance_run_uses_only_catalog_allowlisted_secret_keys(
    ...
    assert "SPARROW_CONFORMANCE_KEYS_JSON" not in captured
...
def test_cli_conformance_status_json_reads_sanitized_store(monkeypatch, capsys, tmp_path):
    from sparrow.cli import main
    from sparrow.conformance import FEATURE_CHAT, STATUS_PASS, ConformanceStore
...
def test_cli_tokenmax_smoke(providers, env, quota, monkeypatch, capsys):
    """`sparrow tokenmax` blasts the fake pool and prints every answer."""
...
def test_cli_tokenmax_passes_timeout(monkeypatch):
    ...
    monkeypatch.setattr("sparrow.cli._read_stdin", lambda: "")
    monkeypatch.setattr(
        "sparrow.tokenmax.select_targets",
...
def test_tokenmax_fan_out_passes_timeout_to_pool_chat():
    from sparrow.tokenmax import fan_out
...
def test_cli_capacity_status_smoke(monkeypatch, capsys):
    ...
    monkeypatch.setenv("SPARROW_KEYS_PATH", "/tmp/sparrow-test-missing-keys.toml")
...
def test_cli_capacity_status_is_cache_first_and_refresh_is_explicit(
    ...
    calls = []
    monkeypatch.setenv("SPARROW_KEYS_PATH", str(tmp_path / "missing-keys.toml"))
    monkeypatch.setattr("sparrow.catalog.load_external_catalog", lambda: [])

    def sync_external_catalog(*, timeout):
        calls.append(timeout)
        return tmp_path / "external.json", []

    monkeypatch.setattr("sparrow.catalog.sync_external_catalog", sync_external_catalog)
...
def _install_capacity_status_edge_fixture(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SPARROW_CONFIG_FILE", str(tmp_path / "config.toml"))
    monkeypatch.setenv("SPARROW_KEYS_PATH", str(tmp_path / "keys.toml"))
    monkeypatch.setenv("SPARROW_QUOTA_PATH", str(tmp_path / "quota.json"))
    monkeypatch.setenv(
        "SPARROW_EXTERNAL_CATALOG_PATH",
        str(tmp_path / "provider_catalog.json"),
    )
    monkeypatch.setattr("sparrow.cli.load_catalog", lambda: catalog)
    monkeypatch.setattr("sparrow.catalog.load_external_catalog", lambda: [])
    monkeypatch.setattr("sparrow.key_inventory.load_inventory", lambda path=None: [])
    monkeypatch.setattr("sparrow.capacity.QuotaStore", lambda: FakeQuota())
    monkeypatch.setattr(
        "sparrow.capacity.effective_env",
...
def test_cli_doctor_smoke(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setenv("SPARROW_CONFIG_FILE", str(tmp_path / "config.toml"))
    monkeypatch.setenv("SPARROW_QUOTA_PATH", str(tmp_path / "quota.json"))
    monkeypatch.setenv("SPARROW_CACHE_PATH", str(tmp_path / "cache.db"))
    monkeypatch.setenv("SPARROW_EXTERNAL_CATALOG_PATH", str(tmp_path / "external.json"))

    assert main(["doctor"]) == 0
    out = capsys.readouterr().out
    assert "sparrow" in out
...
def test_cli_keys_add_confirms_fuzzy_external_match(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setenv("SPARROW_CONFIG", str(user_catalog))
    monkeypatch.setenv("SPARROW_CONFIG_FILE", str(config))
    monkeypatch.setenv("SPARROW_KEYS_PATH", str(inventory))
    answers = iter(["y", "y"])
    monkeypatch.setattr("builtins.input", lambda prompt="": next(answers))
    monkeypatch.setattr("getpass.getpass", lambda prompt="": "secret")

    assert main(["keys", "add", "Hyperbolc"]) == 0
...
def test_cli_keys_add_creates_manual_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SPARROW_CONFIG_FILE", str(config))
    monkeypatch.setenv("SPARROW_KEYS_PATH", str(inventory))
    monkeypatch.setattr("sparrow.cli._load_or_sync_external_catalog", lambda: [])
    answers = iter(["y", "https://api.hyperbolic.xyz/v1", "meta-llama/Llama-3.3-70B-Instruct", "y"])
    monkeypatch.setattr("builtins.input", lambda prompt="": next(answers))
    monkeypatch.setattr("getpass.getpass", lambda prompt="": "secret")

    assert main(["keys", "add", "Hyperbolic"]) == 0
...
def test_cli_keys_add_cloudflare_prompts_for_account_id(tmp_path, monkeypatch, capsys):
    ...
    assert "sparrow providers health -p cloudflare" in out
    assert "python3 -m sparrow" not in out
...
def test_cli_keys_add_autodiscovers_model_when_blank(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sparrow.cli._load_or_sync_external_catalog", lambda: [])
    monkeypatch.setattr(
        "sparrow.catalog.discover_openai_models",
    ...
    assert main(["keys", "add", "Example"]) == 0
...
def test_cli_providers_health_smoke(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "sparrow.cli.cmd_providers_health",
...
def test_dashboard_contains_capacity(monkeypatch):
    ...
    from sparrow.proxy import _dashboard_html
    from sparrow.router import Pool
...
class _FakePool:
    """Minimal stand-in for sparrow.cli.Pool used by the proxy/serve tests.
    ...
    `sparrow.proxy.serve` seam, so this fake can be empty.
    """

    def __init__(self, providers=None):
        # Default to a single fake provider so the "no providers
        # configured" guard in cmd_start doesn't fire. Tests that need
...
def _patch_pool(monkeypatch, providers=None):
    ...
    monkeypatch.setattr(
        "sparrow.cli.Pool", SimpleNamespace(from_default_config=lambda: pool)
    )
    return pool
...
def _patch_serve(monkeypatch, captured=None):
    ...
    monkeypatch.setattr("sparrow.proxy.serve", FakeServer)



def test_cli_tailnet_status_usable(monkeypatch, capsys):
    from sparrow import tailnet
    from sparrow.cli import main
...
def test_cli_tailnet_serve_dry_run_uses_detected_ip(monkeypatch, capsys):
    ...
    assert "SPARROW_BASE_URL=http://100.64.0.5:9999/v1" in out
...
def test_cli_tailnet_serve_refuses_generated_key_without_tty(monkeypatch, capsys):
    ...
    assert "SPARROW_PROXY_KEY" in out
...
def test_cli_tailnet_connect_prints_client_setup(monkeypatch, capsys):
    ...
    assert "SPARROW_BASE_URL=http://laptop.tailnet.local:7777/v1" in out
...
def test_cli_start_tailnet_delegates_to_tailnet_serve(monkeypatch, capsys):
    """`sparrow start --tailnet` must use the same safety logic as `tailnet serve`."""
    from sparrow import tailnet
    from sparrow.cli import main
    ...
    assert main(["start", "--tailnet", "--port", "4242", "--api-key", "abc"]) == 0
    out = capsys.readouterr().err
    assert "100.64.0.5" in out
    assert "4242" in out
...
def test_cli_start_tailnet_refuses_missing_tailscale(monkeypatch, capsys):
    """`start --tailnet` should not silently fall back to loopback when tailscale is missing."""
    from sparrow import tailnet
    from sparrow.cli import main

    monkeypatch.setattr(tailnet.shutil, "which", lambda _: None)
    assert main(["start", "--tailnet", "--port", "8080"]) == 3
    out = capsys.readouterr().err
    assert "tailscale" in out.lower()
    assert "127.0.0.1" in out  # the fallback hint to use loopback


def test_cli_start_refuses_unsafe_non_loopback_bind(monkeypatch, capsys):
    """`start` on a non-loopback, non-Tailnet host without --allow-lan must refuse."""
    from sparrow.cli import main

    # No providers needed — the safety check fires before pool construction.
    assert main(["start", "--host", "192.168.1.10", "--port", "8080"]) == 2
    out = capsys.readouterr().err
    assert "--allow-lan" in out
    assert "192.168.1.10" in out


def test_cli_start_refuses_unsafe_bind_before_missing_provider_error(monkeypatch, capsys):
...
def test_cli_start_allows_unsafe_bind_with_allow_lan_and_key(monkeypatch, capsys):
    """`start --host 192.168.1.10 --allow-lan --api-key K` should reach the server."""
    from sparrow.cli import main
...
def test_cli_start_loopback_no_key(monkeypatch, capsys):
    """`sparrow start` on loopback with no key still works."""
    from sparrow import tailnet
    from sparrow.cli import main
    ...
    assert main(["start"]) == 0  # default host 127.0.0.1, no api key
    out = capsys.readouterr().err
