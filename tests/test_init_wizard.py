...
from sparrow.tailnet import STATE_CLI_MISSING, STATE_USABLE, TailnetStatus
...
def _patch_init(monkeypatch, *, tailnet=True, which=True):
    from sparrow import init_wizard
...
def _isolated_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SPARROW_CONFIG", str(providers))
...
def test_init_metaswarm_tailnet_plan_includes_copy_pastable_commands(
    ...
    assert "sparrow tailnet serve --port 8080" in out
    assert "sparrow profile install metaswarm" in out
    assert (
        "sparrow profile doctor metaswarm --dry-run --base-url http://100.64.0.5:8080"
        in out
    )
    assert "sparrow tailnet connect 100.64.0.5 --port 8080" in out
    assert "OPENAI_BASE_URL=http://100.64.0.5:8080/v1" in out
    setup, foreground, client = _terminal_blocks(out)
    assert "sparrow profile install metaswarm" in setup
    assert _commands(foreground) == ["sparrow tailnet serve --port 8080"]
    assert "sparrow profile doctor metaswarm" in client
...
def test_setup_plans_isolate_foreground_commands_for_local_profiles_tailnet_and_mcp(
    ...
    from sparrow.init_wizard import detect_environment, render_setup_plan
    ...
    cases = (
        (None, False, "sparrow start --port 8080"),
        ("opencode", False, "sparrow start --port 8080"),
        (None, True, "sparrow tailnet serve --port 8080"),
        ("mcp", False, "sparrow mcp"),
    )
    for agent, tailnet, expected_foreground in cases:
        ...
        if agent == "opencode":
            assert "sparrow profile install opencode" in setup
            assert "sparrow profile doctor opencode --dry-run" in client
        if agent == "mcp":
            assert "sparrow proxy" not in out
            assert "sparrow tailnet serve" not in out
