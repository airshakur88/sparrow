...
from sparrow.battle import render_battle_markdown, run_battle, write_battle_record
from sparrow.models import Model, Provider
from sparrow.proxy import _playground_html, serve
...
def test_playground_html_is_unified_data_free_shell_with_protected_fetches():
    ...
    assert "<title>sparrow</title>" in html
...
def test_proxy_battle_endpoint_requires_auth_and_returns_answers(providers, env, quota):
    ...
    try:
        ...
        assert payload["markdown"].startswith("# sparrow battle")
    finally:
        httpd.shutdown()
        httpd.server_close()
