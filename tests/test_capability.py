...
from sparrow import capability as c
...
def test_heuristic_orders_by_size_and_keywords():
    ...
    assert c._heuristic_score("foo-flash") <= 0.40                      
    assert c._heuristic_score("brand-new-unknown") == 0.5                               
...
def test_build_table_precedence_aa_arena_aider():
    ...
    assert table[c.normalize_model_name("frontier-100b")]["source"] == "aa"           
...
def test_fetch_aa_scores_refuses_unsafe_url_without_calling_network(monkeypatch):
    import sparrow.capability as cap
