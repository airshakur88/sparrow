...
import sparrow.conformance as conformance_module
from sparrow.aio import AsyncPool
from sparrow.client import HTTPResult
from sparrow.conformance import (
    _RED_PIXEL,
    FEATURE_CHAT,
    FEATURE_JSON,
    FEATURE_JSON_SCHEMA,
...
from sparrow.errors import NoProvidersConfigured, ProviderHTTPError
...
def test_default_pool_honors_programmatic_conformance_path(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SPARROW_CONFORMANCE_FILE", raising=False)

    pool = Pool.from_default_config(
        env={"SPARROW_CONFORMANCE_FILE": str(path)}
    )
