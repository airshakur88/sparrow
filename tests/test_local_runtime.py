...
from sparrow.local_runtime import (
    LocalRuntime,
...
def test_discovery_is_bounded_headerless_get_with_redirects_disabled(monkeypatch):
    from sparrow import local_runtime
    ...
    check = subprocess.run(
        [
            ...
            (
                ...
                "from sparrow.local_runtime import _NO_REDIRECT_OPENER; "
...
def test_discovery_rejects_self_oversize_and_unsafe_models(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "open",
        lambda *_args, **_kwargs: _Response(b'{"data":[]}', server="sparrow/0.13.0"),
    )
    with pytest.raises(ValueError, match="sparrow proxy"):
...
@pytest.mark.parametrize("model", ("$(touch /tmp/sparrow-pwned)", "`id`"))
...
def test_import_is_pin_only_atomic_idempotent_and_reversible(tmp_path, monkeypatch):
    path = tmp_path / "providers.toml"
    monkeypatch.setenv("SPARROW_CONFIG", str(path))
    runtime = LocalRuntime(
        provider_id="local_lm_studio",
        label="LM Studio",
        base_url="http://127.0.0.1:1234/v1",
...
def test_imported_loopback_provider_loads_without_broad_local_network_opt_in(
    ...
    from sparrow.config import configured_providers, load_catalog
...
def test_catalog_lock_domain_is_independent_of_runtime_environment(tmp_path, monkeypatch):
    ...
    script = (
        "import pathlib, sys; "
        "from sparrow.local_runtime import _catalog_lock; "
...
def test_unrelated_root_directory_flock_cannot_block_catalog_import(tmp_path):
    ...
    import_script = (
        "import pathlib, sys; "
        "from sparrow.local_runtime import LocalRuntime, import_runtime; "
...
def test_readable_home_fallback_uses_private_file_not_directory_lock(tmp_path, monkeypatch):
    ...
    lock_path = home / ".sparrow-catalog.lock"
...
def test_private_runtime_uses_writable_file_when_directory_flock_is_rejected(tmp_path, monkeypatch):
    ...
    assert stat.S_IMODE((runtime / ".sparrow-catalog.lock").stat().st_mode) == 0o600
...
@pytest.mark.skipif(os.name == "nt", reason="Windows symlink privilege is not guaranteed")
...
def test_cli_discover_is_preview_only_and_import_requires_affirmation(
    ...
    monkeypatch.setattr("sparrow.local_runtime.discover_runtime", lambda **_kwargs: runtime)
    ...
    assert "sparrow ask --providers local_lm_studio --model qwen-local hello" in out
    assert "sparrow local remove local_lm_studio --yes" in out
    assert path.exists()
