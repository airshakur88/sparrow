...
from sparrow import task_quality as tq
...
def test_bundled_evidence_contains_only_current_repeated_fixture_results(
    ...
    monkeypatch.setenv(
        "SPARROW_TASK_EVIDENCE_FILE", str(tmp_path / "missing.json")
    )
...
@pytest.mark.parametrize(
    ...
    ids=["code", "markdown", "long-prose", "names", "security", "refactor"],
)
...
def test_task_evidence_requires_current_fixture_trials_and_exact_identity(
    ...
    monkeypatch.setenv("SPARROW_TASK_EVIDENCE_FILE", str(evidence_file))
    tq._evidence_cached.cache_clear()
