...
def _store(tmp_path, day):
    clock = lambda: datetime(2026, 6, day, 12, 0, tzinfo=UTC)              
...
def test_over_budget(tmp_path):
    ...
    assert s.over_budget("groq", "m", rpd=0) is False                      
...
def test_batched_quota_flush_merges_real_processes(tmp_path):
    ...
    import sparrow.quota as quota_module
