...
from sparrow.panel import PanelAnswer, PanelResult
from sparrow.recipes import (
...
def test_recipe_json_files_include_required_schema_fields():
    for path in (ROOT / "src" / "sparrow" / "recipes").glob("*.json"):
        data = json.loads(path.read_text(encoding="utf-8"))
        assert data["schema_version"] == RECIPE_SCHEMA_VERSION
        assert data["version"]
        assert data["example"].startswith("sparrow recipe run ")
...
def test_recipe_run_second_opinion_uses_panel_helper(monkeypatch):
    import sparrow.recipes as recipes
    ...
    assert "sparrow panel" in result.output
...
def test_recipe_run_can_read_stdin(monkeypatch, capsys):
    ...
    monkeypatch.setattr(Pool, "from_default_config", classmethod(lambda cls: FakePool()))
    monkeypatch.setattr("sparrow.cli._read_stdin", lambda: "launch copy from stdin")
...
def test_recipe_run_provider_exhaustion_is_cli_error(monkeypatch, capsys):
    from sparrow.cli import main
    from sparrow.errors import NoProvidersConfigured
    from sparrow.router import Pool
...
def test_recipe_package_data_is_in_build_config():
    ...
    for name in (
        ...
        source = f"src/sparrow/recipes/{name}.json"
