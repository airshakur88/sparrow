...
def readiness_snapshot(
    ...
    *,
    ...
    for provider in providers:
        availability = (
            credential_manager.availability(provider.id) if credential_manager is not None else {}
        )
        managed_configured = bool(availability)
        configured = provider.is_configured(env_copy) or "available" in availability.values()
        ...
        if availability and "available" not in availability.values():
            credential_status = (
                "cooldown" if "cooldown" in availability.values() else
                "disabled" if "disabled" in availability.values() else
                "missing"
            )
            if managed_configured and credential_status == "cooldown":
                configured = True
