...
* **Providers.** Call :func:`register_provider` (or expose a
  ``sparrow.providers`` entry point) to add an endpoint sparrow will
...
def register_provider(provider: Provider) -> None:
    """Add a custom :class:`~sparrow.Provider` to the routing catalog."""
...
def register_adapter(name: str, caller: Callable) -> None:
    ...
    timeout, tools, tool_choice, post) and return a :class:`~sparrow.Reply`.
    """
    ...
    with _ENTRYPOINT_LOCK:
        ...
        try:
            ...
            try:
                ...
                eps = entry_points().get("sparrow.providers", [])  # type: ignore[attr-defined]
            for ep in eps:
                try:
                    obj = ep.load()
