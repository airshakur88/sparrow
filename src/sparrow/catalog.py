...
def default_external_catalog_path() -> Path:
    override = os.environ.get("SPARROW_EXTERNAL_CATALOG_PATH")
    if override:
        return Path(override).expanduser()
    return Path.home() / ".config" / "sparrow" / "provider_catalog.json"
...
def import_external_provider_to_user_catalog(query: str) -> str:
    ...
    try:
        ...
        raise ValueError(
            "external catalog cache is missing; run sparrow catalog sync or "
            "sparrow capacity status first"
        ) from None
