...
def detect_environment(env: dict[str, str] | None = None) -> InitReport:
    ...
    proxy = ProxyConfigStatus(
        ...
        proxy_key_configured=bool(env.get("SPARROW_PROXY_KEY") or cfg.get("proxy_key")),
...
def render_detect_only(report: InitReport, *, port: int = DEFAULT_INIT_PORT) -> str:
    ...
    lines = [
        "sparrow init: environment status",
        "",
        ...
        "  sparrow init --yes --agent opencode",
        f"  sparrow init --yes --agent metaswarm --tailnet --port {port}",
        "  sparrow mcp",
        "",
...
def render_setup_plan(
    ...
    *,
    ...
    serve_cmd = (
        f"sparrow tailnet serve --port {port}" if tailnet else f"sparrow start --port {port}"
    )
    if agent == "mcp":
        serve_cmd = "sparrow mcp"
    lines = [
        "sparrow init: setup plan",
        "",
    ...
    if profile is not None:
        setup_lines.append(f"sparrow profile install {agent}")
    if agent == "mcp":
        setup_lines.extend(
            [
    ...
    if agent == "mcp":
        client_lines.extend(
            [
                ...
                "claude mcp add sparrow -- sparrow mcp",
            ]
        )
    elif agent:
        doctor = f"sparrow profile doctor {agent} --dry-run"
        if tailnet:
            doctor = f"{doctor} --base-url {base}"
        client_lines.append(doctor)
    if tailnet:
        client_lines.append(f"sparrow tailnet connect {host} --port {port}")
        client_lines.append("")
        ...
        client_lines.extend(
            [
                ...
                f"export SPARROW_BASE_URL={base}/v1",
            ]
        )
    if not client_lines:
...
def _provider_catalog_path(env: dict[str, str]) -> Path:
    override = env.get("SPARROW_CONFIG")
    if override:
        return Path(override).expanduser()
