...
_PROFILES: tuple[Profile, ...] = (
    Profile(
            ...
            "opencode.json": json.dumps(
                {
                    ...
                    "provider": {
                        "sparrow": {
                            "name": "sparrow (free pool)",
                            "npm": "@ai-sdk/openai-compatible",
                            "options": {
                                "baseURL": f"{_DEFAULT_PROXY}/v1",
                                "apiKey": "{env:SPARROW_PROXY_KEY}",
        ...
        doctor_checks=[
            ...
            DoctorCheck("binary", "sparrow CLI", "sparrow"),
            DoctorCheck("url", "proxy /v1/models", _DEFAULT_PROXY, path="/v1/models"),
        ],
        notes=(
            "Use sparrow/agent for long-running tool work: it stays on the "
    ...
    Profile(
        ...
        cost_class="paid",
        role_map={
            "escalation": "user-owned paid tool; route through sparrow only when explicitly pointed",
        },
        config_snippets={
            "shell": (
                f"export OPENAI_BASE_URL={_DEFAULT_PROXY}/v1\n"
        ...
        notes=(
            "Codex is a user-owned paid tool. The profile wires it through sparrow "
    ...
    Profile(
        ...
        role_map={
            ...
            "reviewer": "larger reviewer lane via sparrow quality routing",
        ...
        config_snippets={
            ...
            "Tailnet remote agent": (
                ...
                "# (run `sparrow tailnet connect <host> --port 8080` on the client)\n"
        ...
        notes=(
            ...
            "They are never silently routed through sparrow."
        ),
    ),
)
...
def run_doctor_check(
    ...
    *,
    ...
    if check.kind == "url":
        ...
        try:
            data = None
            headers = {"User-Agent": "sparrow-doctor/1.0"}
            if proxy_key:
