        ...
        try:
            discovered = set(
                discover_openai_models(
                    provider.base_url,
                    api_key=api_key if api_key is not None else provider.api_key(pool.env),
    ...
    *,
    ...
    def run(provider) -> BenchRow | None:
        ...
        if manager is not None and provider.adapter == "openai":
            selected = manager.reserve(provider.id, model or "*", CredentialOperation.DISCOVERY)
            if isinstance(selected, CredentialUnavailable):
                return BenchRow(provider.id, False, None, None, selected.detail)
            discovery_selection = selected
        mname, skip_note = _pick_model_for_health(
            provider,
            model,
            pool,
            timeout,
            discovery_selection.secret if discovery_selection else None,
        )
        if skip_note:
        ...
        if manager is not None:
            selected = manager.reserve(provider.id, mname, CredentialOperation.PROBE)
            if isinstance(selected, CredentialUnavailable):
                return BenchRow(key, False, None, None, selected.detail)
            selection = selected
        try:
            reply = _client.call(
                provider,
                ...
                api_key=selection.secret if selection else provider.api_key(pool.env),
                env=pool.env,
            ...
            if manager is not None and selection is not None:
