...
def default_conformance_path(env: Mapping[str, str] | None = None) -> Path:
    ...
    return Path.home() / ".config" / "sparrow" / "conformance.json"
...
def run_target_canaries(
    ...
    *,
    ...
    if credential_manager is not None and not provider.keyless:
        selected_credential = credential_manager.reserve(
            provider.id,
            model,
            CredentialOperation.PROBE,
        )
        if isinstance(selected_credential, CredentialUnavailable):
            return {
                feature: {
    ...
    for feature in selected:
        try:
            ...
            if (
                ...
                and credential_manager is not None
                and selection is not None
                and isinstance(exc, ProviderHTTPError)
            ):
                ...
                if failure is not None:
                    credential_manager.record_failure(selection, failure)
