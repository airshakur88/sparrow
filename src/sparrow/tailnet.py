"""Tailscale Tailnet detection and safe-serving helpers for sparrow.
...
- generate session-only bearer tokens when the user has not configured
  ``SPARROW_PROXY_KEY`` (or ``[settings].proxy_key``)
...
def generate_session_token(*, nbytes: int = 24) -> str:
    ...
    ``SPARROW_PROXY_KEY``/``[settings].proxy_key`` but is serving on
...
def safe_base_url(host: str, port: int) -> str:
    ...
    to a reverse proxy / Tailscale HTTPS feature, not to sparrow.
    """
    url_host = host
