...
Quality routing (``SPARROW_ROUTING=quality``) matches prompt *difficulty* to
...
MIT-mirrored) mapped onto catalog model names. ``sparrow capability sync``
...
def user_capability_path() -> Path:
    ...
    Honors ``SPARROW_CAPABILITY_FILE`` for overrides and test injection.
    """
