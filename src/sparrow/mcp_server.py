...
_SERVER_INSTRUCTIONS = (
    ...
    "INVOKE THESE AS MCP TOOLS DIRECTLY. Do NOT shell out to the `sparrow` CLI (e.g. "
    "spawning `sparrow mcp` or `sparrow tokenmax` as a subprocess) to reach them — "
    ...
    '`sparrow tokenmax "<prompt>"` in their own terminal.'
)
...
TOOLS = [
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "routing": {
                    ...
                    "description": "How to pick the model: agent (strongest healthy tier with quota spreading), quality (capability matched to prompt), spread (whole-pool breadth), fast (lowest latency), fair (provider balance), apex (strongest-first), swift (lightweight/low-latency), adaptive (difficulty-aware), or auto (server default).",
                },
                "task": {
                    "type": "string",
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "routing": {
                    "type": "string",
                    "enum": list(PUBLIC_ROUTING_ALIASES),
                    "description": "How to rank candidate panel models (default: quality); apex is strongest-first, swift is lightweight/low-latency, and adaptive is difficulty-aware.",
                },
                "max_tokens": {
                    "type": "integer",
    ...
    },
    {
        ...
        "description": (
            ...
            "`sparrow start` on another machine. Includes the local Tailnet IPv4, "
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "routing": {
                    "type": "string",
                    "enum": list(PUBLIC_ROUTING_ALIASES),
                    "description": "How to rank eligible targets (default: the pool's routing mode); apex is strongest-first, swift is lightweight/low-latency, and adaptive is difficulty-aware.",
                },
            },
            "required": ["prompt"],
        },
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "routing": {
                    "type": "string",
                    "enum": list(PUBLIC_ROUTING_ALIASES),
                    "description": "Routing mode to explain (default: the server's mode); apex is strongest-first, swift is lightweight/low-latency, and adaptive is difficulty-aware.",
                },
                "task": {
                    "type": "string",
                    "enum": list(TASK_HINTS),
    ...
    },
    {
        ...
        "description": "List the available free provider/model ids sparrow can route to.",
    ...
    },
    {
        "name": "free_llm_stats",
        "description": (
            "Show sparrow's LIFETIME totals (persisted across restarts): tokens served "
...
def _tool_tailnet_info(args: dict) -> dict:
    ...
    if status.usable:
        ...
        lines.append(f"  sparrow start --host {status.ipv4} --port {port}")
        lines.append("")
        ...
        lines.append("")
        lines.append(
            "(Token placeholder only — sparrow start prints the real token "
        ...
        lines.append("")
        lines.append(
        "sparrow will still serve on loopback for local-only use. "
...
def _tool_tokenmax(pool: Pool, args: dict, notify=None) -> dict:
    ...
    routing = _routing_arg(args.get("routing"))
    picks, n_providers = select_targets(
        pool,
        msgs,
        args.get("max_models"),
        routing=routing,
    ...
    head = [
        ...
        f"{n_providers} providers using {routing or pool.routing} routing; "
        f"{len(answered)} answered, {len(failed)} unavailable. "
        f"{RAINBOW_BANNER}",
        ...
        if method == "initialize":
            ...
            return _result(
                mid,
                {
                    ...
                    "capabilities": {"tools": {}},
        "serverInfo": {"name": "sparrow", "version": version},
