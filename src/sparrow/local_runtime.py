    ...
    *,
    ...
    try:
        with _NO_REDIRECT_OPENER.open(request, timeout=timeout) as response:
            ...
            if "sparrow" in server.lower():
                raise ValueError("endpoint is a sparrow proxy, not a local model runtime")
...
def _markers(provider_id: str) -> tuple[str, str]:
    return (
        f"# sparrow-local-runtime-begin: {provider_id}",
        f"# sparrow-local-runtime-end: {provider_id}",
    )
...
def _stable_catalog_lock(path: Path | None = None) -> Iterator[None]:
    ...
    lock_name = ".sparrow-catalog.lock"
    lock_fd = -1
    locked = False
    try:
