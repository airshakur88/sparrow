...
from .credentials import CredentialOperation, CredentialSelection
from .errors import (
...
class AsyncPool:
    ...
    async def _reserve_credential(self, manager, provider_id, model, excluded_ids, deadline):
        ...
        *,
        ...
        while pending or deferred or fallback:
            ...
            if lease is None:
                ...
                continue
            if manager is not None and not target.provider.keyless:
                reserved = await self._reserve_credential(
                    manager,
                    target.provider.id,
                    target.model,
                    excluded_credentials.setdefault(target.provider.id, set()),
                    deadline,
                )
                if not isinstance(reserved, CredentialSelection):
                    non_ctx_failure = True
                    attempts.append((target.name, "skipped (no credential available)"))
                    continue
                selection = cast(CredentialSelection, reserved)
            ...
            try:
                ...
                if selection is not None and manager is not None:
                    failure = classify_credential_failure(exc.status)
                    if failure is not None:
                        await asyncio.to_thread(manager.record_failure, selection, failure)
                        excluded_credentials.setdefault(target.provider.id, set()).add(
                ...
                if selection is not None and manager is not None:
                    ...
                    if failure is not None:
                        await asyncio.to_thread(manager.record_failure, selection, failure)
                        excluded_credentials[target.provider.id].add(selection.credential_id)
