"""Command-line interface for sparrow.

sparrow ask "question"        one-shot completion (reads stdin too)
sparrow tokenmax "question"   fan out to eligible targets, synthesize the swarm
sparrow providers            list configured / available providers
sparrow quota                show today's per-provider usage
sparrow start                run the OpenAI-compatible proxy server
"""
...
def cmd_ask(args: argparse.Namespace) -> int:
    ...
    if args.role and role is None:
        print(
            f"sparrow: unknown role '{args.role}'\n",
            file=sys.stderr,
        )
...
"(run sparrow init --yes to inspect configuration).",
            file=sys.stderr,
        )
        return 3
    ...
    wise = is_wise_enabled(pool_env, override=args.mode, settings=mode_settings)
    routing_for_mode = (
        routing_override(args.routing) if getattr(args, "routing", None) is not None else None
    )
    if not has_routing_config:
        if wise:
    ...
    if not picks:
        print("sparrow: no models available to blast", file=sys.stderr)
        return 3
    if wise:
...
def cmd_providers(args: argparse.Namespace) -> int:
    ...
    if not configured:
        print("\nNo providers configured yet. Run sparrow init --yes to inspect configuration.")
    return 0
...
def cmd_stats(args: argparse.Namespace) -> int:
    ...
    print("sparrow — lifetime (served free):")
...
def cmd_keys_usage(args: argparse.Namespace) -> int:
    ...
    print(render_usage(
        CredentialStore(), provider=args.provider, day=args.day, as_json=args.json,
...
def cmd_keys_add(args: argparse.Namespace) -> int:
    ...
    if args.credential_id:
        config_path = register_credential(
            default_config_path(), provider=provider.id, credential_id=args.credential_id,
            env_var=primary_env_var,
            quota_group=args.quota_group or "shared", secret=value, enabled=not args.disabled,
        )
    else:
...
def cmd_local_discover(args: argparse.Namespace) -> int:
    ...
    if args.name or args.base_url:
        if not args.name:
            print("sparrow: --base-url requires --name", file=sys.stderr)
            return 2
        try:
...
def cmd_local_import(args: argparse.Namespace) -> int:
    ...
    if not args.yes:
        print(
            "sparrow: local import changes the user catalog; review `local discover` "
    ...
    try:
        ...
        print(
            f"sparrow: local catalog update failed ({type(exc).__name__})",
            file=sys.stderr,
        )
        return 3
    ...
    print("Revert:")
    print(f"  sparrow local remove {runtime.provider_id} --yes")
    return 0
...
def cmd_local_remove(args: argparse.Namespace) -> int:
    ...
    if not args.yes:
        print("sparrow: local remove requires --yes", file=sys.stderr)
        return 2
    try:
        ...
        return 3
    if not removed:
        print(f"sparrow: no managed local runtime named {args.provider_id}", file=sys.stderr)
        return 3
...
def cmd_catalog_status(args: argparse.Namespace) -> int:
    ...
    if not providers:
        ...
        print("Run: sparrow catalog sync")
        return 1
...
def cmd_capability_sync(args: argparse.Namespace) -> int:
    ...
    aa_key = os.environ.get("SPARROW_AA_API_KEY")
    ...
    if not aa_key:
        print(
            "  Artificial Analysis skipped — set SPARROW_AA_API_KEY for much broader "
...
def cmd_conformance_run(args: argparse.Namespace) -> int:
    ...
    if not features or unknown or len(set(features)) != len(features):
        ...
        print(f"sparrow: invalid conformance feature selection{detail}", file=sys.stderr)
        return 2
    catalog = _runtime_catalog()
    try:
        ...
        print(
            "sparrow: invalid bounded conformance key map",
            file=sys.stderr,
        )
        return 2
    ...
    if args.include_disabled:
        if not args.provider or not args.model or args.providers or args.all_models:
            print(
                "sparrow: --include-disabled requires one exact --provider and --model",
                file=sys.stderr,
            )
            return 2
        ...
        if model is None:
            print(
                "sparrow: exact disabled canary target is missing or not configured",
                file=sys.stderr,
            )
            return 3
        if model.enabled:
            print(
                "sparrow: --include-disabled is only for a model that is off by default",
                file=sys.stderr,
            )
            return 2
        ...
        if args.provider:
            print(
                "sparrow: --provider is reserved for exact --include-disabled canaries; "
    ...
    if not targets:
        print("sparrow: no configured provider/model matched the conformance filters", file=sys.stderr)
        return 3
...
def _conformance_env(catalog) -> dict[str, str]:
    ...
    env = effective_env()
    raw = os.environ.get("SPARROW_CONFORMANCE_KEYS_JSON")
    env.pop("SPARROW_CONFORMANCE_KEYS_JSON", None)
    if not raw:
        return env
    if len(raw) > 65_536:
...
def cmd_conformance_status(args: argparse.Namespace) -> int:
    ...
    if not targets:
        print("No protocol conformance evidence yet. Run `sparrow conformance run`.")
        return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    # `--tailnet` is a Tailnet-safe alias for `sparrow tailnet serve`.
    # It runs the same safety logic while keeping the proxy startup command
    # under the stable `sparrow start` verb.
    ...
    if not loopback and not proxy_key:
        ...
        print(
            f"sparrow: WARNING — binding to {args.host} (not loopback) with NO proxy key "
...
def _run_tailnet_serve(
    *,
    ...
    """Shared body for ``sparrow tailnet serve`` and ``start --tailnet``.
    ...
    # --api-key > SPARROW_PROXY_KEY > config proxy_key > generate one.
    explicit_key = (
        api_key
        or os.environ.get("SPARROW_PROXY_KEY")
        or settings().get("proxy_key")
    ...
    if dry_run:
        ...
        print(
            f"sparrow tailnet serve (dry run)\n"
...
"(run sparrow init --yes to inspect configuration).",
    ...
    if generated_session_token:
        ...
        if explicit_key is None:  # pragma: no cover - defensive invariant
            print(
                "sparrow: failed to generate a one-session proxy key.",
                file=sys.stderr,
            )
            return 2
        try:
    ...
    n_models = sum(len(p.models) for p in pool.providers)
    print(
        f"sparrow tailnet serve on {base_url}/v1  "
        f"({len(pool.providers)} providers, {n_models} models)\n"
        f"  auth status    : {auth_status}\n"
...
def cmd_tailnet_status(args: argparse.Namespace) -> int:
    ...
    if status.state == STATE_USABLE:
        print(
            f"sparrow tailnet: usable\n"
            f"  IPv4           : {status.ipv4}\n"
            f"  next           : `sparrow tailnet serve --port {args.port}` "
    ...
    print(
        f"sparrow tailnet: {label}\n"
        f"  reason         : {status.detail}\n"
        f"  fallback       : `sparrow start` on 127.0.0.1 still works.",
        file=sys.stderr,
    )
    return 1
...
def cmd_tailnet_connect(args: argparse.Namespace) -> int:
    ...
    host = args.host
    if not host:
        print("sparrow: tailnet connect needs a tailnet host or IP", file=sys.stderr)
        return 2

    base = safe_base_url(host, args.port)
    print(
        f"sparrow tailnet connect: {host}:{args.port}\n"
...
def cmd_battle(args: argparse.Namespace) -> int:
    ...
    if result.truncated:
        print(
            f"sparrow: battle ran {len(result.answers)} model(s) "
    ...
    if provider_count < 3:
        print(
            f"sparrow: only {provider_count} configured provider(s) available for battle",
            file=sys.stderr,
        )
...
def cmd_playground(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        pass
    print(
        f"sparrow playground: no proxy reachable on {base}. "
        f"Start one with `sparrow start --port {args.port}`, then open {base}/playground.",
        file=sys.stderr,
    )
    return 3
...
def cmd_recipe_show(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"sparrow recipe: {exc}", file=sys.stderr)
        return 3
    if args.json:
    ...
    try:
        if args.recipe:
            ...
            if args.role and not args.prompt:
                print(
                    "sparrow jobs add: --role requires a prompt",
                    file=sys.stderr,
                )
                return 2
            spec_payload = {
            ...
            print(
                "sparrow jobs add: provide --recipe <name> [prompt] or --role <role> with a prompt",
                file=sys.stderr,
            )
            return 2
        store = JobStore()
        ...
        print(f"sparrow jobs add: {exc}", file=sys.stderr)
        return 3
...
def cmd_jobs_run(args: argparse.Namespace) -> int:
    ...
    if args.limit is not None and args.limit < 1:
        print("sparrow jobs run: --limit must be >= 1", file=sys.stderr)
        return 3
    if args.max_failures is not None and args.max_failures < 1:
        print("sparrow jobs run: --max-failures must be >= 1", file=sys.stderr)
        return 3
    ...
    if args.dry_run:
        ...
        try:
            ...
            print(f"sparrow jobs run: {exc}", file=sys.stderr)
            return 3
        print(plan)
        return 0
    try:
...
def cmd_jobs_cancel(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"sparrow jobs cancel: {exc}", file=sys.stderr)
        return 3
    except JobError as exc:
        print(f"sparrow jobs cancel: {exc}", file=sys.stderr)
        return 3
...
def cmd_report_last(args: argparse.Namespace) -> int:
    ...
    record = store.last()
    if record is None:
        print("sparrow report: no run records found", file=sys.stderr)
        return 3
...
def cmd_report_open(args: argparse.Namespace) -> int:
    ...
    if not path.exists():
        print(f"sparrow report: report not found: {path}", file=sys.stderr)
        return 3
    open_report_path(path)
    return 0
...
def cmd_cost_show(args: argparse.Namespace) -> int:
    ...
    if record is None:
        print(
            f"sparrow cost: run not found: {args.run_id}. "
            "Run `sparrow report list` to see available runs.",
            file=sys.stderr,
        )
        return 3
...
def cmd_profile_show(args: argparse.Namespace) -> int:
    ...
    if profile is None:
        print(f"sparrow: unknown profile '{args.name}'", file=sys.stderr)
        return 3
...
def cmd_profile_doctor(args: argparse.Namespace) -> int:
    ...
    if not math.isfinite(args.timeout) or args.timeout <= 0:
        print(
            "sparrow profile doctor: timeout must be a finite positive number",
            file=sys.stderr,
        )
        return 3
    if args.base_url:
        try:
            ...
            print("sparrow profile doctor: invalid --base-url", file=sys.stderr)
            return 3
    if args.dry_run:
        print(render_doctor_plan(profile))
        return 0
    env = effective_env()
    proxy_key = env.get("SPARROW_PROXY_KEY") or settings().get("proxy_key")
...
def cmd_code(args: argparse.Namespace) -> int:
    ...
    if out is None:
        print(f"sparrow: unknown agent '{args.agent}'\n", file=sys.stderr)
...
def cmd_mcp(args: argparse.Namespace) -> int:
    ...
    pool = Pool.from_default_config()
    print(
        f"sparrow MCP server (stdio) — {len(pool.providers)} providers ready. "
        "Add this command to your MCP client configuration.",
        file=sys.stderr,
    )
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_ask.add_argument(
        "-r",
        "--role",
        help="use a role preset (see `sparrow roles`)",
    )
    p_ask.add_argument(
        "--routing",
    ...
    p_ask.add_argument(
        "--mode",
        choices=["normal", "wise"],
        help="per-command quota mode override (default: SPARROW_MODE or config)",
    )
    p_ask.add_argument(
        "--second-opinion",
        action="store_true",
    ...
    p_jobs_cancel.add_argument("job_id", help="job id from `sparrow jobs list`")
    ...
    p_cost_show.add_argument("run_id", help="run id from `sparrow report list`")
    ...
    p_start.add_argument(
        "--api-key",
        default=None,
        help="require this Bearer token on requests (or set SPARROW_PROXY_KEY)",
    )
    p_start.add_argument(
        "--tailnet",
        action="store_true",
        help="alias for `sparrow tailnet serve` — bind to the local 100.x Tailnet IPv4, require auth",
    )
    p_start.add_argument(
        "--allow-lan",
        action="store_true",
    ...
    p_tailnet = sub.add_parser(
        "tailnet",
        help="Tailnet gateway: serve sparrow over a Tailscale 100.x address",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    configure_logging_from_env()  # honor SPARROW_LOG=<level> for the CLI/proxy
    parser = build_parser()
