...
usage() {
    printf '%s\n' \
        'Update Sparrow with uv.' \
        'Usage: curl -fsSL https://raw.githubusercontent.com/airshakur88/sparrow/refs/heads/main/update.sh | sh'
}
...
if ! command -v sparrow >/dev/null 2>&1; then
    printf '%s\n' 'sparrow command was not found after update.' >&2
    exit 1
fi
printf '%s\n' 'Sparrow updated from GitHub. User configuration and data were preserved.'
sparrow --help >/dev/null
